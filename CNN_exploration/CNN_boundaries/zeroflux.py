...
def CellEquation(State, t, Input, A, B, z):
	...
	ExpandedM[0][0] = Input[0][0] # top left corner
	ExpandedM[0][ExpandedM.shape[1] - 1] = Input[0][Input.shape[1] - 1] # top right corner
	ExpandedM[ExpandedM.shape[0] - 1][0] = Input[Input.shape[0] - 1][0] # bottom left corner
	ExpandedM[ExpandedM.shape[0] - 1][ExpandedM.shape[1] - 1] = Input[Input.shape[0] - 1, Input.shape[1] - 1] # bottom right corner

	ExpandedM[0, 1 : Input.shape[1] + 1] = Input[0][:] # top boundary
	ExpandedM[1 : Input.shape[0] + 1, 0] = Input[:][0] # left boundary
	ExpandedM[ExpandedM.shape[0] - 1, 1 : Input.shape[1] + 1] = Input[Input.shape[0] - 1][:] # bottom boundary
	ExpandedM[1 : Input.shape[0] + 1, ExpandedM.shape[1] - 1] = Input[:][Input.shape[1] - 1] # right boundary
	...
	ExpandedState[0][0] = StateMat[0][0] # top left corner
	ExpandedState[0][ExpandedState.shape[1] - 1] = StateMat[0][StateMat.shape[1] - 1] # top right corner
	ExpandedState[ExpandedState.shape[0] - 1][0] = StateMat[StateMat.shape[0] - 1][0] # bottom left corner
	ExpandedState[ExpandedState.shape[0] - 1][ExpandedState.shape[1] - 1] = StateMat[StateMat.shape[0] - 1, StateMat.shape[1] - 1] # bottom right corner

	ExpandedState[0, 1 : StateMat.shape[1] + 1] = StateMat[0][:] # top boundary
	ExpandedState[1 : StateMat.shape[0] + 1, 0] = StateMat[:][0] # left boundary
	ExpandedState[ExpandedState.shape[0] - 1, 1 : StateMat.shape[1] + 1] = StateMat[StateMat.shape[0] - 1][:] # bottom boundary
	ExpandedState[1 : StateMat.shape[0] + 1, ExpandedState.shape[1] - 1] = StateMat[:][StateMat.shape[1] - 1] # right boundary
