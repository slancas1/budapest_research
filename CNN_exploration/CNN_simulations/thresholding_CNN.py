...
def CellEquation(State, t, Input, A, B, z):
	...
	resizedInput = np.reshape(Input, [InputImg.shape[0], InputImg.shape[1]])
	shapeX = np.reshape(State, [InputImg.shape[0], InputImg.shape[1]])
	Output = np.zeros(InputImg.shape)
	Nonlinear = nonLinearity(shapeX)
	for x in range(1, InputImg.shape[0] - 1):
		for y in range(1, InputImg.shape[1] - 1):
			negX = np.multiply(-1, shapeX[x, y])
			StateRegion = Nonlinear[(x - 1) : (x + 2), (y - 1) : (y + 2)]
			Ay = np.sum(np.multiply(A, nonLinearity(StateRegion)))
			Bu = np.sum(np.multiply(B, resizedInput[(x - 1) : (x + 2), (y - 1) : (y + 2)]))
			Output[x, y] = negX + Ay + Bu + z
